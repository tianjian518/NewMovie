...
function TmdbPanel() {
  ...
  const load = () => api.getSettings().then((s) => setKey(s["tmdb_api_key"] || "")).catch(() => {});
  useEffect(() => { load(); }, []);

  async function save() {
    await api.saveSettings({ tmdb_api_key: key });
    setMsg("已保存（重启无需重填；留空则跳过 TMDB）");
  }

  return (
    <section>
      <h2 className="text-xl font-bold mb-3">TMDB API Key</h2>
...
function StoragePanel() {
  ...
  const [form, setForm] = useState({ name: "main", type: "openlist", base_url: "http://openlist:5244", token: "", sign_key: "", rate_limit: 2 });
  const [msg, setMsg] = useState("");
  ...
  return (
    <section>
      ...
      <div className="bg-card rounded-lg p-4 space-y-3 max-w-xl">
        <input className="w-full bg-ink rounded p-2" placeholder="名称" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
        ...
        <input className="w-full bg-ink rounded p-2" type="number" placeholder="限速 req/s" value={form.rate_limit} onChange={(e) => setForm({ ...form, rate_limit: Number(e.target.value) })} />
      ...
      <ul className="mt-3 text-sm text-gray-400">
        {list.map((s) => <li key={s.id}>{s.name} · {s.base_url} · {s.type}</li>)}
      </ul>
    </section>
  );
}
